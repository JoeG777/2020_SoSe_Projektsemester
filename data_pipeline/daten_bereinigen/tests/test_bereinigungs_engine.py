...
class test_imputation(unittest.TestCase):
    ...
    def test_low_threshold_throws_exception(self):
        self.assertRaises(exc.InvalidConfigValueException, be.imputation, test_imputation.test_data, "-1S")
    ...
    def test_imputation_works_correctly(self):
        ...
        imp_frame["from"] = [timestamp1]
        imp_frame["to"] = [timestamp2]
        pd.testing.assert_frame_equal(imp_frame, be.imputation(test_imputation.test_data, "3600S"))
    ...
    def test_imputation_no_gaps_works_correctly(self):
        ...
        imp_frame = pd.DataFrame(columns=["from", "to"])
        pd.testing.assert_frame_equal(imp_frame.astype("float64"), be.imputation(test_data_no_gaps, "3600S"))
...
class test_remove_gaps(unittest.TestCase):
    ...
    def test_remove_gaps_works_with_empty_imputation_dict(self):
        ...
        self.assertEqual(be.remove_gaps(test_data, pd.DataFrame()).all(), test_data.all())
    ...
    def test_remove_gaps_works_correctly(self):
        ...
        self.assertEqual(be.remove_gaps(test_data, test_remove_gaps.imp_frame).all(), expected_output.all())

    def test_remove_gaps_empty_data_throws_exception(self):
        self.assertRaises(exc.NoDataException, be.remove_gaps,  pd.Series({}, dtype="float64"), test_remove_gaps.imp_frame)
